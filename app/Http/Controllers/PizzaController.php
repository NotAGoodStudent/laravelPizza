...
class PizzaController extends Controller
{
    ...
    public function validateCredentials()
    {
       /* $validate = request()->validate([
        ...
        if($validate)
        {*/
            $p = $this->addPizzaData(request('type'),request('ingredients'), request('crust'), request('price'));
            $p->save();
            return 0;
  /*      }
        return 1;*/
    ...
    public function addPizzaData($type, $ingredients, $crust, $price)
    {
        $ingrarr = explode(',', $ingredients);
        $pizza = new Pizza();
        $pizza->type = $type;
        $pizza->ing1 = $ingrarr[0];
        $pizza->ing2 = $ingrarr[1];
        $pizza->ing3 = $ingrarr[2];
        $pizza->crust = $crust;
        $pizza->price = $price;
